...
export class OffLedger {
    public static ToStruct(buffer: Buffer): IOffLedger {
        ...
        for (let i = 0; i < numArguments; i++) {
            ...
            args.push({ key: key.toString(), value: value });
        }
        ...
        for (let i = 0; i < numBalances; i++) {
            ...
            balances.push({ color: Buffer.from(colorBytes), balance: balance });
        }
    ...
    public static ToBuffer(req: IOffLedger): Buffer {
        ...
        if (req.requestType !== undefined && [0, 1].includes(req.requestType)) {
        ...
        if (req.arguments) {
            req.arguments.sort((lhs: any, rhs: any) => lhs.key.localeCompare(rhs.key));
        ...
        if (req.publicKey !== undefined) buffer.writeBytes(req.publicKey);
